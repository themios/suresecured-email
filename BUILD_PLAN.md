...
5. **Twilio as an alternate provider (added 2026-07-30).** Some customers already run texting through their own Twilio account — Settings → Phone & SMS now has a Telnyx/Twilio toggle. Twilio is BYO per tenant (their own Account SID, Auth Token — encrypted at rest, same masked-on-reload pattern as the SMTP password — and phone number), unlike Telnyx's shared-platform-account model. `cron.js`'s SMS dispatch branches on the tenant's chosen provider through the same interface (`src/lib/twilio.js` mirrors `lib/telnyx.js`'s shape). Twilio's inbound webhook (`/twilio-hooks/sms`, `/twilio-hooks/status`) gets the same STOP/HELP/consent/delivery-receipt treatment as Telnyx — verified end to end including HMAC signature verification (per-tenant, since there's no single platform webhook secret when credentials are BYO).
6. **10DLC registration** remains an external step per tenant (3-7 day approval, Telnyx only — Twilio has its own equivalent A2P 10DLC process) — inbound replies and STOP/HELP work immediately regardless; outbound is gated on it.
7. **Metering** is not yet built — still open, lower priority than the compliance/consent piece above.
